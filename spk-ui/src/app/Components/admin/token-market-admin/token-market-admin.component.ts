...
export class TokenMarketAdminComponent implements OnInit {
  ...
  load = async () => {
    const spkDetails: TokenModel = await this.token.spkDetail().call( { from: this.account } )
    this.userBalance = {
      etherBal: await this.spec.getBalance( this.account ),
      tokenBal: ( await this.token.balance(this.account).call( { from: this.account } ) / ( 10 ** spkDetails.tokenDecimals ) )
    }
    this.specToken = {
