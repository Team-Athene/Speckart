...
export class UserGuard implements CanActivate, CanActivateChild, CanLoad {
  ...
  async canActivate( next: ActivatedRouteSnapshot ): Promise<boolean> {
    ...
    const userType = await this.web3var.spk.checkUser( userID ).call( {
    ...
    if ( userType === '1' ) {
      localStorage.clear()
      console.log("INSIDE")
      const user = await this.web3var.spk.consumer().call( { from: this.web3var.account } )
      console.log( 'Log: UserGuard -> constructor -> user', user )
      return true
    } else {
      localStorage.clear()
